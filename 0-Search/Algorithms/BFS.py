# An implementation of the Breadth-First Search (BFS) algorithm for solving search problems.
...
def BFS(
    ...
    if starting_node.state == problem.goal_state:
        ...
        return None, None
    bfs_frontier = Queue_Frontier([starting_node])
    ...
    while True:
        ...
        # Otherwise, remove the first node from the frontier (queue)
        # and add it to the explored nodes set.
        current_node = bfs_frontier.remove_node()
        ...
        for node in future_nodes:
            ...
            elif not bfs_frontier.contains_state(
                node.state
            ) and not explored.contains_state(node.state):
                bfs_frontier.add_node(node)
