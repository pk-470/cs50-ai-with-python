# An implementation of the A* algorithm for solving search problems.
...
def A_star(
    ...
    if starting_node.state == problem.goal_state:
        ...
        return None, None
    a_star_frontier = Informed_Frontier([starting_node])

    # Initialise the explored nodes set.
    explored = Explored_Nodes()

    # Define the function that chooses the node of minimum
    # (distance + path cost from the goal).
    def choice(nodes):
        node = nodes[0]
        for n in nodes[1:]:
            if heuristic(n) + n.path_cost < heuristic(node) + node.path_cost:
                node = n
    ...
    while True:
        ...
        # Otherwise, remove the node determinded to have the smallest distance
        # from goal + path cost according to the chosen heuristic, and add it
        # to the explored nodes set.
        current_node = a_star_frontier.remove_node(choice)
        ...
        for node in future_nodes:
            ...
            elif not a_star_frontier.contains_state(
                node.state
            ) and not explored.contains_state(node.state):
                a_star_frontier.add_node(node)
